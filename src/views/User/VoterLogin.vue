  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-100 p-4">
    <div class="w-full max-w-md bg-white p-8 rounded-xl shadow-2xl">
      ...
      <div class="mt-6 text-center space-y-2">
        ...
        <router-link :to="{ name: 'AdminLogin' }" class="text-xs text-gray-500 hover:text-blue-600">
          Admin Login
        </router-link>
      </div>
      </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (voterId.value === 'user' && password.value === 'ssgvote') {
      localStorage.setItem('isUser', 'true'); 
      router.push({ name: 'VotingDashboard' }); 
    } else {
      throw new Error("Invalid ID or password. Please check your credentials.");
    }

  } catch (error) {
    loginError.value = error.message || 'A network error occurred. Please try again.';
  } finally {
